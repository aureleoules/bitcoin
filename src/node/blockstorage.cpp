...
{
    assert(m_prune_mode && nManualPruneHeight > 0);
...
{
    LOCK2(cs_main, cs_LastBlockFile);
    if (chain_tip_height < 0 || m_prune_target == 0) {
        return;
    }
    ...
    if (nCurrentUsage + nBuffer >= m_prune_target) {
        ...
        for (int fileNumber = 0; fileNumber < m_last_blockfile; fileNumber++) {
            ...
            if (nCurrentUsage + nBuffer < m_prune_target) { // are we below our target?
                break;
            }
    ...
    LogPrint(BCLog::PRUNE, "target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d blk/rev pairs\n",
             m_prune_target / 1024 / 1024, nCurrentUsage / 1024 / 1024,
             ((int64_t)m_prune_target - (int64_t)nCurrentUsage) / 1024 / 1024,
             nLastBlockWeCanPrune, count);
}
...
bool BlockManager::FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, CChain& active_chain, uint64_t nTime, bool fKnown)
{
    ...
    if (!fKnown) {
        ...
        if (bytes_allocated != 0 && m_prune_mode) {
