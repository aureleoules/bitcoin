...
struct KeyConverter {
    ...
    std::optional<Key> FromPKBytes(I first, I last) const {
        ...
        return xonly_pubkey.GetCPubKey();
    }
...
void Test(const std::string& ms, const std::string& hexscript, int mode, const KeyConverter& converter,
          int opslimit = -1, int stacklimit = -1, std::optional<uint32_t> stack_exec = {})
{
          ...
          int opslimit = -1, int stacklimit = -1, std::optional<uint32_t> stack_exec = {})
{
    ...
    Test(ms, hexscript, mode, CONVERTER, opslimit, stacklimit);
    CONVERTER.SetContext(miniscript::MiniscriptContext::TAPSCRIPT);
    Test(ms, hextapscript == "=" ? hexscript : hextapscript, mode, CONVERTER, opslimit, stacklimit, stack_exec);
}
} // namespace
...
BOOST_AUTO_TEST_CASE(fixed_tests)
{
    ...
    BOOST_CHECK(ms_stack_ok && ms_stack_ok->CheckStackSize(CONVERTER));
    ...
    BOOST_CHECK(ms_stack_nok && !ms_stack_nok->CheckStackSize(CONVERTER));
    Test(ms_stack_limit, "?", "?", TESTMODE_VALID | TESTMODE_NONMAL | TESTMODE_NEEDSIG | TESTMODE_P2WSH_INVALID, 4 * count + 1, 1, 1 + count + 1);
